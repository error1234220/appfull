...
import { ChatMessage, MessageAuthor, ChatSession, ImageFile, Gem, GemInstructions, UserProfile, AIProfile, MemoryItem, MemoryProposal, AiTool, GeneratedImage, DetectedObject, CustomizationSettings, StoryState, StorySetup, Theme, RolePlaySetup, FavoritePrompt, Bookmark, CodeBlock, CodeModificationType, CodeSnippet, SmartSuggestion, Workflow, WorkflowExecutionState, UserInterestProfile, AIGirlfriendProfile, ModelName, StorageInfo, StorageStats, PassionWeaverStory, PassionWeaverSetup, PassionWeaverChoice, PassionWeaverPageVisual, AppIdeaSetup, AppIdeaResult, Emotion, EmotionScores, LiveCharacterState, ObjectOfDesireSetup, AnatomyExplorerSetup, View, StudioCategory, StudioTopic, StudioTopicContent, EighteenPlusTalkSettings, SexualProfileSetup, SexualProfileAnalysis } from './types';
import { 
...
const App: React.FC = () => {
  ...
  const activeAiProfile = useMemo(() => {
    // Safety override: if NSFW mode is off, always use the safe, customizable profile.
    if (!settings.isNsfwModeEnabled) {
      if (settings.activeAiModel !== 'safe') {
  ...
  }, [settings.isNsfwModeEnabled, settings.activeAiModel, customizableAiProfile, updateSetting]);
...
}, [activeChatId, sessions, gems, userProfile, activeAiProfile, useMemory, memories, handleSaveMemory, matchUserStyle, userInterestProfile, updateSession, settings.isNsfwModeEnabled, updateEmotionScores]);
  ...
  const handleSend = useCallback(async () => {
    ...
    const userMessage: ChatMessage = { id: `user-${Date.now()}`, author: MessageAuthor.USER, content: prompt, imageFile };
    const currentSession = sessions.find(s => s.id === activeChatId);
    const isFirstMessage = currentSession?.messages.length === 0;
    
    updateSession(activeChatId, session => ({ ...session, messages: [...session.messages, userMessage] }));
    const messagesForApi = [...(currentSession?.messages || []), userMessage];
    
    if (isFirstMessage && !currentSession?.gemId) {
        const title = await generateTitleForChat(prompt || "Image analysis", settings.model);
    ...
    setIsLoading(false);
    setActiveTool('none');
  }, [activeChatId, prompt, imageFile, sessions, processApiStream, activeTool, settings.language, settings.model, updateSession]);
  ...
  const handleGenerateWithNanoBanana = useCallback(async (prompt: string, remixImage?: ImageFile) => {
    ...
    try {
        ...
        if (err instanceof Error) setImageGenerationError(err.message);
        else setImageGenerationError("An unknown error occurred with Nano Banana.");
        return [];
    } finally {
        setIsGeneratingImage(false);
        setGeneratingImageCount(0);
  ...
  const handleSendRolePlayMessage = useCallback(async (prompt: string) => {
    ...
    const stream = generateContentStream(messagesForApi, systemInstruction, null, 'none', false, settings.model, undefined, undefined);
    let fullResponse = '';
    ...
    setIsLoading(false);
  }, [rolePlaySetup, rolePlayMessages, settings.model, updateEmotionScores]);
...
const handleSendGirlfriendMessage = useCallback(async (prompt: string) => {
    ...
    const stream = generateContentStream(messagesForApi, systemInstruction, null, 'none', false, settings.model);
    ...
    setIsLoading(false);
}, [activeGirlfriendId, aiGirlfriends, userProfile, handleGenerateImages, settings.model, updateEmotionScores]);
  ...
  const storageStats: StorageStats = useMemo(() => ({
    ...
    }, [anatomyExplorerSetup, anatomyExplorerMessages, settings.model, updateEmotionScores]);
  ...
  const handleSelectFunZoneTopic = useCallback(async (topic: StudioTopic, category: StudioCategory) => {
    ...
    useEffect(() => {
        ...
        handleShowStorage, handleShowSettings, handleShowShortcuts, handleCancelView
    ]);
